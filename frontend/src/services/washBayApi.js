...
export async function getWashBays(params = {}) {
  ...
  const url = `${API_BASE_URL}/wash-bays${
...
export async function getWashBayById(id) {
  const res = await fetch(`${API_BASE_URL}/wash-bays/${id}`, {
    method: "GET",
    headers: getAuthHeaders(),
  });
...
export async function updateWashBay(id, payload) {
  const res = await fetch(`${API_BASE_URL}/wash-bays/${id}`, {
    method: "PATCH",
    headers: getAuthHeaders(),
    body: JSON.stringify(payload),
